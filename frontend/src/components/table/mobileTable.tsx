...
export function MobileTable({ data, columns, entityName, idKey, onLoadMore, hasMore = false, isLoadingMore = false }: GenericTableProps) {
  ...
  return (
    <div className="table-container d-flex flex-column align-items-center w-100 h-100">
          ...
          <tr>
            <td colSpan={columns.length}>
              <div className="d-flex justify-content-center gap-3">
                ...
                <div className="search_box d-flex align-items-center" ref={searchBoxRef}>
                  {displaySearchInput && (
      ...
      <div className="mobile-table-scroll-container" ref={scrollContainerRef}>
                ...
                (query.trim().length >= 2 && searchedItems && searchedItems.length > 0)
                  ...
                  (searchedItems as unknown as TableData[]).map((item, index) => (
                    <tr
                      ...
                      className="table-row d-flex flex-column"
                      onClick={() => navigate(`${entityName}?code=${item[idKey] ?? item.code}`)}>
                      {columns.map((column, colIndex) => {
                        const value = String((item as Record<string, unknown>)[column.key as string] ?? '');
                        return (
                          <td
                            key={column.key}
                            style={{
